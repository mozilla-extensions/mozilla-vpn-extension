...
describe('Serverlist Templates', () => {
    ...
    test("serverList renders calls the subListTemplate with a city and renders the result", ()=>{
        ...
        `;
      const element = document.createElement('div');
      render(countrylistHolder([{name: "city1"}, {name: "city2"}],subListTemplate),element);
    ...
    test("countryListItem renders", ()=>{
        const element = document.createElement('div');
        expect(element.innerHTML).toBe("");
        render(countryListItem(
            testCountry,
            false,
            (country) => {},
            (city)=>{return html`<p class="city">${city.name}</p>`}
        ),element);
        // It should render the subtemplate
        expect(element.querySelector(".city").textContent).toBe(testCountry.cities[0].name);
        // There should be a flag loaded
        expect(element.querySelector("img").src).toContain("assets/flags/" + testCountry.code.toUpperCase() + ".png");
