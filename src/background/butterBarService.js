 ...
 export class ButterBarService extends Component {
   ...
   async init() {
     ...
     this.vpnController.state.subscribe((s) => {
      ...
      // if interventions... 
      this.#mButterBarList.value.push(new ButterBarAlert(
        "conflictingProgram",
    ...
    this.conflictObserver.conflictingAddons.subscribe((conflictingAddons) => {  
      ...
      if (conflictingAddons.length <= 1 || this.dismissedAlerts.find((alertId) => alertId == id)) {
        console.log("No conflicting addons found or alert already dismissed. No butter bar alert necessary.")
        return;
      }
  ...
  removeAlert(id) {
    const newAlertList = this.#mButterBarList.value.filter(({alertId}) => alertId !== id)
