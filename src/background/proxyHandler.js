...
export class ProxyHandler extends Component {
  ...
  async excludeOrigin(origin) {
    ...
    return this.updateSiteContexts(siteContext);
  }

  /**
   * Retrieves a Mullvad proxy url for a specified location.
   ...
   */
  async getMullvadProxy(countryCode, cityName) {
    const proxyServer = this.pickServerBasedOnWeight(await this.getServersForLocation(countryCode, cityName));
    return this.parseProxy(this.getMullvadProxyUrl(proxyServer.socksName));
  }

  /**
   * Constructs a Mullvad proxy URL from a socks name.
   * @param {string} socksName - The socks name.
   */
  getMullvadProxyUrl(socksName) {
  ...
  /**
   * Retrieves an array of the Mullvad servers for a given city.
   * @param {string} countryCode - The country code.
   * @param {string} cityName - The city name.
   */
  async getServersForLocation(countryCode, cityName) {
    const servers = await Utils.getServers();
    const serverCountry = servers.find(({code}) => code === countryCode);
  ...
  /**
   * Implements a weighted random selection to select a server from a list of servers.
   * @param {Array} serverList - The list of servers.
   */
   pickServerBasedOnWeight(serverList) {
    const filteredServerList = serverList.filter(server => server.socksName);
  ...
  /**
   * Takes in an Object with an origin, cityName, and countryCode. 
   * Gathers proxy information for the given location and attaches it
  ...
  async setContextForOrigin(info) {
    const proxyInfo = await this.getMullvadProxy(info.countryCode, info.cityName);
  ...
  async updateSiteContexts(siteContext) {
    const {siteContexts} = await Utils.getSiteContexts();
